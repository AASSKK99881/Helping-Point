...
const handleSearch = async () => {
  ...
  try {
    const res = await axios.get(`/api/tasks/search?keyword=${keyword.value}`, {
      headers: { Authorization: getToken() }
    });
...
th, td { padding: 12px; text-align: left; border-bottom: 1px solid #eee; overflow: hidden; text-overflow: ellipsis; }
th { background-color: #f8f9fa; font-weight: bold; }

/* 列宽控制 */
th:nth-child(1) { width: 50px; } /* ID */
th:nth-child(2) { width: 30%; }  /* 标题内容 */
th:nth-child(3) { width: 80px; } /* 积分 */
th:nth-child(7) { width: 80px; } /* 操作 */

.task-title { font-weight: bold; color: #333; margin-bottom: 4px; }
.task-content { font-size: 12px; color: #888; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.points { color: #f57c00; font-weight: bold; }

/* 状态标签样式 */
.status-pending { color: #2196F3; background: #e3f2fd; padding: 2px 6px; border-radius: 4px; font-size: 12px; }
.status-accepted { color: #ff9800; background: #fff3e0; padding: 2px 6px; border-radius: 4px; font-size: 12px; }
.status-completed { color: #4caf50; background: #e8f5e9; padding: 2px 6px; border-radius: 4px; font-size: 12px; }
